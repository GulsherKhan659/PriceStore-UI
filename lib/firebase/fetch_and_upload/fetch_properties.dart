...
class FetchProperties{
  ...
  static Stream<List<PropertyBeanClass>>? homeProperties(){
   try{
     return FirebaseFirestore.instance.collection("Properties").where('isApproved',isEqualTo: true).limit(3).snapshots().map(
             (event) {
     ...
     print("Exception Found In homeProperties Method in fetch_properties.dart");
   }
  }
  /// homeProperties Stream Method is to get All properties in HomeScreen
  static Stream<List<PropertyBeanClass>>? allProperties(){
    try{
      return FirebaseFirestore.instance.collection("Properties").where('isApproved',isEqualTo: true).snapshots().map(
              (event) {
            List<PropertyBeanClass> _listOfProperty = <PropertyBeanClass>[];
            event.docs.forEach((element) {
              _listOfProperty.add(PropertyBeanClass.fromJson(element.data()));
  ...
  static Stream<List<PropertyBeanClass>>? FilteredProperty({required String city,required int min,required int max,required String property_type,required String isBuy,required String n_bath,required String n_bedroom}){
    try{
      return FirebaseFirestore.instance.collection("Properties").where('isApproved',isEqualTo: true)
